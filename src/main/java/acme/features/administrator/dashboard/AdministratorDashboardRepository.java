...
public interface AdministratorDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT i.price.currency,avg(i.price.amount),i.technology FROM Item i WHERE i.type = 0 GROUP BY i.price.currency, i.technology")
	List<String> averageRetailPriceOfComponentsByTechnologyCurrency();
	
	@Query("SELECT i.price.currency,stddev(i.price.amount),i.technology FROM Item i WHERE i.type = 0 GROUP BY i.price.currency, i.technology")
	List<String> deviationRetailPriceOfComponentsByTechnologyCurrency();
	
	@Query("SELECT i.price.currency,min(i.price.amount),i.technology FROM Item i WHERE i.type = 0 GROUP BY i.price.currency, i.technology")
	List<String> minimumRetailPriceOfComponentsByTechnologyCurrency();
	
	@Query("SELECT i.price.currency,max(i.price.amount),i.technology FROM Item i WHERE i.type = 0 GROUP BY i.price.currency, i.technology")
	List<String> maximumRetailPriceOfComponentsByTechnologyCurrency();
	
	@Query("SELECT count(i) FROM Item i WHERE i.type = 1")
	Integer totalNumberOfTools();
	
	@Query("SELECT i.price.currency,avg(i.price.amount) FROM Item i WHERE i.type = 1 GROUP BY i.price.currency")
	List<String> averageRetailPriceOfToolsByCurrency();
	
	@Query("SELECT i.price.currency,stddev(i.price.amount) FROM Item i WHERE i.type = 1 GROUP BY i.price.currency")
	List<String> deviationRetailPriceOfToolsByCurrency();
	
	@Query("SELECT i.price.currency,min(i.price.amount) FROM Item i WHERE i.type = 1 GROUP BY i.price.currency")
	List<String> minimumRetailPriceOfToolsByCurrency();
	
	@Query("SELECT i.price.currency,max(i.price.amount) FROM Item i WHERE i.type = 1 GROUP BY i.price.currency")
